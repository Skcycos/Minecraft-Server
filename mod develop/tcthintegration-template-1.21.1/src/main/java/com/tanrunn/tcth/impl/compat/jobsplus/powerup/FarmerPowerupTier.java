...
/**
 * Activation tier of a tcth:farmer ability-route node (phase 4B).
 *
 * <p>Pure value type — no Jobs+/Arc dependency. Route semantics: only the
 * highest <em>active</em> node of a route applies; lower nodes of the same
 * route are excluded (no stacking, no summing).
 */
public enum FarmerPowerupTier {

    /** No node of the route is active (or the player has no tcth:farmer job). */
    NONE,
