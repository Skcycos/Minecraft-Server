...
/**
 * Per-player anti-farm cooldown for the farmer livestock route (phase 4B).
 *
 * <p>In-memory only: keys are player UUIDs, values are server tick counts when
 * the livestock effects were last successfully granted. Nothing is ever written
 * to player NBT or world data.
 *
 * <ul>
 *   <li>cooldown duration is configurable ({@code farmerLivestockCooldownTicks},
 *       default 400 ticks = 20 s);</li>
 *   <li>all three livestock nodes share the same cooldown;</li>
 ...
 * </ul>
 */
public final class FarmerLivestockCooldown {

    private static final FarmerLivestockCooldown INSTANCE = new FarmerLivestockCooldown();
    ...
    private LongSupplier tickSource = FarmerLivestockCooldown::serverTickOrZero;

    /** Cooldown ticks; production reads {@link Config#FARMER_LIVESTOCK_COOLDOWN_TICKS}. */
    private IntSupplier cooldownTicksSupplier = () -> Config.FARMER_LIVESTOCK_COOLDOWN_TICKS.get();

    private FarmerLivestockCooldown() {
    }

    public static FarmerLivestockCooldown instance() {
        return INSTANCE;
    }
    ...
    public void registerLifecycle(IEventBus gameBus) {
        ...
        synchronized (this) {
            ...
            gameBus.addListener(FarmerLivestockCooldown::onPlayerLogout);
            gameBus.addListener(FarmerLivestockCooldown::onServerStopping);
        }
    }

    /**
     * Whether the player is still inside the livestock cooldown window.
     */
    ...
    /**
     * Records a successful grant for the player. Called by the
     * {@code tcth:farmer_livestock_effects} reward only after at least one
     * status effect was actually applied (success-driven commit).
     */
    ...
    private static void onServerStopping(ServerStoppingEvent event) {
        INSTANCE.clearAll();
        TCTHIntegration.LOGGER.debug("[TCTH] Farmer livestock cooldown cache cleared (server stopping)");
    }
    ...
    public static void resetForTesting() {
        ...
        INSTANCE.tickSource = FarmerLivestockCooldown::serverTickOrZero;
        INSTANCE.cooldownTicksSupplier = () -> Config.FARMER_LIVESTOCK_COOLDOWN_TICKS.get();
